...
get_latest_release() {
  curl -Ls --silent "https://github.com/220-dev/figma-linux-font-helper/releases/latest" | perl -ne 'print "$1\n" if /v([0-9]{1,3}\.[0-9]{1,3}\.[0-9]{1,4})/' | head -1;
}

get_latest_release_link_download() {
  local latest=$(get_latest_release);
  echo "http://github.com/220-dev/figma-linux-font-helper/releases/download/v${latest}/fonthelper.tar.xz"
}
